...
          let showName = rawName.replace(/[\(\)]/g, '').trim(); 
          // ⭐️ แก้ไขเพิ่ม: หากชื่อเริ่มต้นด้วย 'ID ' แสดงว่าไม่มีชื่อจริง ให้ใช้แค่ ID ตัวเลข
          if (showName.startsWith('ID ')) {
              showName = `#${c.id}`; // ใช้ ID เป็นรูปแบบที่ปลอดภัยกว่า
          } else {
              showName = showName;
          }
          
